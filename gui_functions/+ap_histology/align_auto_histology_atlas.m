...
if user_confirm_flag
    user_confirm = questdlg('Clear existing alignments and auto-align?','Confirm','Yes','No','No');
...
% Remove any existing alignments
histology_ccf_fieldnames = fieldnames(AP_histology_processing.histology_ccf);
histology_ccf_removefields = intersect(histology_ccf_fieldnames, ...
    {'atlas2histology_tform','atlas2histology_size','control_points'});

AP_histology_processing.histology_ccf = ...
    rmfield(AP_histology_processing.histology_ccf, ...
    histology_ccf_removefields);

% Update status
histology_guidata.update([],[],histology_gui,'Auto-aligning atlas/histology slices...');

% Grab slice images and grayscale
% (use clim settinsg from histology scroller, apply rigid transform)
...
for curr_slice = 1:length(histology_guidata.data)
    curr_slice_bw = ...
        ...
        permute(diff(histology_guidata.clim,[],2),[2,3,1])).* ...
        cast(permute(histology_guidata.channel_visibility,[2,3,1]), ...
        class(histology_guidata.data{curr_slice})),[],3);
...
% % (UNUSED: Binarize slices - to outline only outline)
