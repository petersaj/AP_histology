function annotate_volume(~,~,histology_scroll_gui)
...
function add_area(currentObject, eventdata, gui_fig, annotate_fcn)
        ...
        if ~isfield(AP_histology_processing.annotation,'volume')
            AP_histology_processing.annotation.area = ...
                struct('label',cell(0),'vertices',cell(0));
        end
        
        % Find probe index in annotations, add segment
        area_idx = find(strcmp(volume_label,[AP_histology_processing.annotation.area.label]));
        if isempty(area_idx)
            area_idx = length(AP_histology_processing.annotation.probe)+1;
            AP_histology_processing.annotation.area(area_idx).label = volume_label;
            AP_histology_processing.annotation.area(area_idx).vertices = ...
                cell(size(histology_scroll_guidata.data));
        end

        AP_histology_processing.annotation.area(area_idx).vertices{histology_scroll_guidata.curr_im_idx} = ...
