...
% Convert CCF to IBL coordinates standard: [ML,AP,DV] in microns
ibl_bregma_ccf = [540, 44, 570];
probe_line_fits_ibl = cellfun(@(x) ...
    (x(:,[3,1,2]) - ibl_bregma_ccf([3,1,2])) * 10, ...
    {probe_line_fits.ccf},'uni',false);

for curr_probe = 1:length(probe_line_fits_ibl)

    probe_line_fits_ibl{curr_probe}(:,2) = -1 * probe_line_fits_ibl{curr_probe}(:,2);
    probe_line_fits_ibl{curr_probe}(:,3) = -1 * probe_line_fits_ibl{curr_probe}(:,3);
